...
# Carregar variáveis do .env para obter nomes dos usuários
try:
    from dotenv import load_dotenv
except ImportError:
    import sys
    st.error("O pacote python-dotenv é necessário. Instale com: pip install python-dotenv")
    sys.exit(1)

load_dotenv(os.path.join(os.path.dirname(__file__), '..', '.env'))

# Mapear usuários do .env
usuarios = {
    "170293": os.getenv("USUARIO_170293"),
    "79979": os.getenv("USUARIO_79979"),
    "134266": os.getenv("USUARIO_134266"),
    "179319": os.getenv("USUARIO_179319"),
    "157757": os.getenv("USUARIO_157757"),
    "165824": os.getenv("USUARIO_165824"),
    "192731": os.getenv("USUARIO_192731"),
    "175779": os.getenv("USUARIO_175779"),
    "170614": os.getenv("USUARIO_170614"),
    "192479": os.getenv("USUARIO_192479"),
    "128524": os.getenv("USUARIO_128524"),
    "165826": os.getenv("USUARIO_165826"),
    "192482": os.getenv("USUARIO_192482"),
}
usuario_opcoes = [f"{nome} ({id})" for id, nome in usuarios.items() if nome]

st.set_page_config(page_title="Painel Técnico – Todos os Usuários", layout="wide")
st.title("🏷️ Painel Técnico – Todos os Usuários")
...
# Seletor de usuário
usuario_escolhido = st.selectbox("👤 Escolha o usuário", usuario_opcoes)
user_id_escolhido = usuario_escolhido.split("(")[-1].replace(")", "").strip()
st.info(f"Usuário selecionado: {usuarios[user_id_escolhido]}")
...
# Filtrar apenas pelo usuário selecionado
df = df[df["user_id"].astype(str) == user_id_escolhido]
...
# Como não há mais filtro por setor, vamos considerar todos os equipamentos do usuário selecionado
# Se quiser um critério diferente, ajuste aqui:
esperado_prev_mensal = equipamentos_df.shape[0]

mensal_realizados = 0
semestral_realizados = 0

# === Preventiva Mensal (apenas mês atual, igual lógica das corretivas) ===
st.markdown("---")
st.subheader("🗓️ Preventiva Mensal")

# Garantir que a coluna 'data' é datetime
if not pd.api.types.is_datetime64_any_dtype(df_filt["data"]):
    df_filt["data"] = pd.to_datetime(df_filt["data"], errors="coerce")

# Filtra tarefas mensais do mês atual
df_mensal = df_filt[
    (df_filt["tipo"].str.contains("Preventiva Mensal", case=False, na=False)) &
    (df_filt["data"].dt.month == hoje.month) &
    (df_filt["data"].dt.year == hoje.year)
]

total_tarefas_mensal = len(df_mensal)
mensal_esperados = sum(len(row.get("equipamentos_id") or []) for _, row in df_mensal.iterrows())
mensal_realizados = sum(
    len({q.get("questionnaireEquipamentId") for q in (row.get("questionarios") or []) if q.get("questionnaireEquipamentId")})
    for _, row in df_mensal.iterrows()
)

colm1, colm2, colm3 = st.columns(3)
colm1.metric("📋 Total de Tarefas Mensais", total_tarefas_mensal)
colm2.metric("🔧 Equipamentos em Mensais", mensal_esperados)
colm3.metric("✅ Equipamentos Atendidos", mensal_realizados)

# === Preventiva Semestral (apenas mês atual, igual lógica das corretivas) ===
st.markdown("---")
st.subheader("📅 Preventiva Semestral")
...
total_tarefas_semestral = len(df_semestral)
semestral_esperados = sum(len(row.get("equipamentos_id") or []) for _, row in df_semestral.iterrows())
...
cols1.metric("📋 Total de Tarefas Semestrais", total_tarefas_semestral)
cols2.metric("🔧 Equipamentos em Semestrais", semestral_esperados)
cols3.metric("✅ Equipamentos Atendidos", semestral_realizados)

# 🔻 Linha de baixo – Acompanhamento de Corretivas
st.markdown("---")
st.subheader("🔧 Corretivas")
...
col7.metric("📋 Total de Tarefas Corretivas", total_tarefas_corretivas)
col8.metric("🔧 Equipamentos em Corretivas", corretiva_esperados)
col9.metric("✅ Equipamentos Atendidos", corretiva_realizados)
...
if st.button("📈 Gerar Gráfico Consolidado do Setor"):
    ...
    # Dados de equipamentos esperados
    equipamentos_esperados = [mensal_esperados, semestral_esperados, corretiva_esperados]
    
    # Dados de equipamentos realizados/atendidos
    equipamentos_realizados = [mensal_realizados, semestral_realizados, corretiva_realizados]
    
    # Criar DataFrame para o gráfico
    df_grafico = pd.DataFrame({
        'Categoria': categorias,
        'Equipamentos Esperados': [esperado_prev_mensal, esperado_prev_mensal, esperado_prev_mensal],  # Usar o mesmo valor para todos
        'Equipamentos Realizados': equipamentos_realizados
    })
    
    # Calcular porcentagem de realização
    df_grafico['Porcentagem Realizada'] = (df_grafico['Equipamentos Realizados'] / 
                                         df_grafico['Equipamentos Esperados'] * 100).round(1)
    
    # Substituir NaN por 0
    df_grafico = df_grafico.fillna(0)
    
    # Exibir tabela de dados
    st.write(f"### Dados Consolidados do {usuarios.get(user_id_escolhido, user_id_escolhido)}")
    ...
    st.write(f"### Gráfico de Equipamentos por Tipo de Tarefa - {usuarios.get(user_id_escolhido, user_id_escolhido)}")
    ...
    with col_graf1:
        ...
        chart_data = pd.DataFrame({
            'Preventiva Mensal': [esperado_prev_mensal, mensal_realizados],
            'Preventiva Semestral': [esperado_prev_mensal, semestral_realizados],
            'Corretiva': [total_tarefas_corretivas, corretiva_realizados]  # Usar número de tarefas corretivas
        }, index=['Ativos', 'Realizados'])
    ...
    with col_graf2:
        ...
        porcentagem_data = pd.DataFrame({
            'Porcentagem': df_grafico['Porcentagem Realizada'].values
    ...
    with col_pizza1:
        ...
        qtd_tarefas = [esperado_prev_mensal, esperado_prev_mensal, total_tarefas_corretivas]
        ...
        for tipo, valor in zip(tipos_tarefas, [esperado_prev_mensal, esperado_prev_mensal, total_tarefas_corretivas]):
            cor = cores[tipo]
            if tipo == 'Corretiva':
    ...
    st.markdown(f"**Total de equipamentos no setor:** {esperado_prev_mensal}")
    st.markdown(f"**Total de tarefas no período:** {len(df_filt)}")
    st.markdown(f"**Preventivas Mensais:** {len(df_mensal)} tarefas / {mensal_realizados} equipamentos atendidos ({round(mensal_realizados/mensal_esperados*100 if mensal_esperados else 0, 1)}%)")
    st.markdown(f"**Preventivas Semestrais:** {len(df_semestral)} tarefas / {semestral_realizados} equipamentos atendidos ({round(semestral_realizados/semestral_esperados*100 if semestral_esperados else 0, 1)}%)")
    st.markdown(f"**Corretivas:** {len(df_corretiva)} tarefas / {corretiva_realizados} equipamentos atendidos ({round(corretiva_realizados/corretiva_esperados*100 if corretiva_esperados else 0, 1)}%)")
    ...
    porcentagem_consolidada = round((total_consolidado / esperado_prev_mensal) * 100, 1) if esperado_prev_mensal > 0 else 0
...
if col_btn1.button("📊 Gerar Relatório de Faturamento"):
    ...
    st.download_button(
        ...
        file_name=f"faturamento_{usuarios.get(user_id_escolhido, user_id_escolhido)}_{data_ini.strftime('%d-%m-%Y')}_a_{data_fim.strftime('%d-%m-%Y')}.xlsx",
...
if col_btn2.button("📂 Gerar Relatório Detalhado por Escola"):
    ...
    st.download_button(
        ...
        file_name=f"detalhado_{usuarios.get(user_id_escolhido, user_id_escolhido)}_{data_ini.strftime('%d-%m-%Y')}_a_{data_fim.strftime('%d-%m-%Y')}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
