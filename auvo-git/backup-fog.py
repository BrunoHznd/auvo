...
# Ajustando caminhos para os bancos de dados
import os
...
# Controle de exibição da animação
if 'animacao_exibida' not in st.session_state:
    st.session_state['animacao_exibida'] = False

if not st.session_state['animacao_exibida']:
    ...
    logo_path = Path("marbr.png")
    if not gif_path.exists():
        st.error(f"Arquivo não encontrado: {gif_path}")
        st.stop()
    if not logo_path.exists():
        st.error(f"Arquivo não encontrado: {logo_path}")
        st.stop()
    ...
    st.markdown(
        ...
        .rocket-container {{
            ...
            pointer-events: none; z-index: 9999; margin: 0; padding: 0; overflow: hidden;
        }}
        .rocket-img {{
        ...
        @keyframes takeoff {{
            0% {{transform: translateX(-50%) rotate(0deg) scale(0.5);bottom: 20px;opacity: 1;filter: brightness(1);}}
            40% {{transform: translateX(-50%) rotate(0deg) scale(0.7);bottom: calc(100% - 150px);opacity: 1;filter: brightness(1.5) drop-shadow(0 0 15px #ff6600);}}
            50% {{bottom: calc(100% - 200px);}}
            50.1%, 60% {{transform: translateX(-50%) rotate(0deg) scale(0.7);bottom: calc(100% - 200px);opacity: 1;filter: brightness(1.5) drop-shadow(0 0 15px #ff6600);}}
            60.1% {{transform: translateX(-50%) rotate(0deg) scale(0.7);bottom: calc(100% - 200px);opacity: 1;filter: brightness(1.5) drop-shadow(0 0 15px #ff6600);}}
            90% {{transform: translateX(-50%) rotate(360deg) scale(0.7);bottom: 25%;opacity: 1;filter: brightness(1.3) drop-shadow(0 0 10px #ff6600);}}
            100% {{transform: translateX(-50%) rotate(360deg) scale(0.6);bottom: 20px;opacity: 0.9;filter: brightness(1) drop-shadow(0 0 5px #ff6600);}}
        }}
        </style>
        <div class="rocket-container">
            <img src="data:image/gif;base64,{encoded_gif}" class="rocket-img" id="rocket-img">
            <img src="data:image/png;base64,{encoded_logo}" class="logo-img" id="logo-img">
        </div>
        <script>
        setTimeout(function() {{
            document.getElementById('rocket-img').classList.add('fadeout');
        }}, 15000); // 12s animação + 3s pousado = 15s
        setTimeout(function() {{
            document.getElementById('logo-img').classList.add('fadein');
        }}, 17000); // 2s fadeout foguete, logo fadein
        setTimeout(function() {{
            document.getElementById('rocket-img').style.display = 'none';
        }}, 17000); // remove foguete
        </script>
        """,
        unsafe_allow_html=True
    )
    import time
    time.sleep(17)  # Aguarda o tempo total da animação e transição
    st.session_state['animacao_exibida'] = True
    st.rerun()

# Após a animação, exibe o logotipo fixo no local do foguete
if st.session_state.get('animacao_exibida', False):
    import base64
    from pathlib import Path
    logo_path = Path("marbr.png")
    if logo_path.exists():
        ...
        st.markdown(
            ...
            <div class=\"rocket-container\">
                <img src=\"data:image/png;base64,{encoded_logo}\" class=\"logo-pequeno\" alt=\"Logotipo\">
            </div>
            """,
            unsafe_allow_html=True
        )

# A partir daqui, código normal do dashboard:
...
# Calcula a diferença corretamente: Total - Mensal - Semestral
total_mensal = len(ids_equipamentos_realizados)
total_semestral = len(ids_equipamentos_semestral)

# Fórmula correta: Total Equipamentos - Prev. Mensal Realizada - Prev. Semestral Realizada
diferenca = qtde_equipamentos_setor - total_mensal - total_semestral

# Formata o valor para exibição
if diferenca < 0:
    # Se a diferença for negativa, formata com sinal de +
    faltam_mensal = f"+{-diferenca}"  # Exibe como string com +
    label_faltam = "⚠️ Excesso Prev. Mensal"
elif diferenca == 0:
    # Se for zero, mostra 0
    faltam_mensal = "0"
    label_faltam = "⚠️ Faltam Prev. Mensal"
else:
    # Se for positivo, mostra o valor normal
    faltam_mensal = str(diferenca)
    label_faltam = "⚠️ Faltam Prev. Mensal"
...
<div class="mensal-card">
    ...
    <div style="display: flex; justify-content: space-between; margin-top: 10px;">
        <div style="text-align: center; flex: 1;">
            <div style="font-size: 0.9em; color: #ffffff;">📋 Total Equipamentos</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ffffff;">{qtde_equipamentos_setor}</div>
        </div>
        <div style="text-align: center; flex: 1;">
            <div style="font-size: 0.9em; color: #ffffff;">✅ Prev. Mensal Realizada</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ffffff;">{mensal_realizados}</div>
        </div>
        <div style="text-align: center; flex: 1;">
            <div style="font-size: 0.9em; color: #ffffff;">{label_faltam}</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ffffff;">{faltam_mensal}</div>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="semestral-card">
    ...
    <div style="display: flex; justify-content: space-between; margin-top: 10px;">
        <div style="text-align: center; flex: 1;">
            <div style="font-size: 0.9em; color: #ffffff;">📋 Total Previsto</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ffffff;">{semestral_previsto}</div>
        </div>
        <div style="text-align: center; flex: 1;">
            <div style="font-size: 0.9em; color: #ffffff;">✅ Prev. Semestral Realizada</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ffffff;">{semestral_realizados}</div>
        </div>
        <div style="text-align: center; flex: 1;">
            <div style="font-size: 0.9em; color: #ffffff;">⚠️ Faltam Prev. Semestral</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ffffff;">{max(semestral_previsto - semestral_realizados, 0)}</div>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="corretiva-card">
    ...
    <div style="display: flex; justify-content: space-between; margin-top: 10px;">
        <div style="text-align: center; flex: 1;">
            <div style="font-size: 0.9em; color: #ffffff;">📋 Total de Tarefas Corretivas</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ffffff;">{total_tarefas_corretivas}</div>
        </div>
        <div style="text-align: center; flex: 1;">
            <div style="font-size: 0.9em; color: #ffffff;">🔧 Equipamentos em Corretivas</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ffffff;">{corretiva_esperados}</div>
        </div>
        <div style="text-align: center; flex: 1;">
            <div style="font-size: 0.9em; color: #ffffff;">✅ Equipamentos Atendidos</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ffffff;">{corretiva_realizados}</div>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
